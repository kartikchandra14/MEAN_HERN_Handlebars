...
app.post("/login", async(req,res) => {
    try{
        ...
        if(req.body.email && req.body.password){
            ...
            const isEmailExist = await StudentModel.findOne({email: emailPassed});
            console.log('POST_login1',isEmailExist );
            if(isEmailExist){
                const passwordPassed = isEmailExist[0].password;
                const isPasswordMatch = await bcryptjs.compare(req.body.password, passwordPassed)

                const tokenGeneratd = await isEmailExist.generateAuthToken();// after password match generation of token 
