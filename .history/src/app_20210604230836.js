    ...
    try{
        ...
        if(req.body.email && req.body.password){
            ...
            if(specificStudentData){
                const passwordPassed = specificStudentData[0].password;
                const isPasswordMatch = await bcryptjs.compare(req.body.password, passwordPassed)

                const tokenGeneratd = await specificStudentData[0].generateAuthToken();// after password match generation of token 
