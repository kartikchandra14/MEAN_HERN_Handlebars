        ...
        if(req.body.email && req.body.password){
            ...
            if(specificStudentData){
                ...
                if(isPasswordMatch){
                    ...
                    await specificStudentData.tokens.push({token: tokenGeneratd});
                    res.render('index');
...
app.get("/logout",auth, async (req,res) => {
    try{
        // console.log('POST_logout==>', req.cookies.jwt);
        
        //========== 1>> for single user logout ==============
        // req.user.tokens = req.user.tokens.filter((element) =>{ //index is 2nd , array is 3rd & 'this' is 4th parameter of filter method
        //     return element.token !== req.token;
        // })
        // ========== 2>>for mutiple user logout ==============
        req.user.tokens = [];

        res.clearCookie("jwt");
        // console.log('logout successfully')
        await req.user.save();
        res.render("login"); //render secret.hbs
    }
    catch(err){
